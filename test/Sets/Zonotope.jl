...
for N in @tN([Float64, Float32, Rational{Int}])
    ...
    if isdefined(@__MODULE__, :StaticArrays)
        ...
        Z2 = convert(Zonotope, Hyperrectangle(SVector{2}(c), SVector{2}(r)))  # static arrays
        @test isidentical(Z2, Zonotope(SVector{2}(c), SMatrix{2,2}(G)))
    end
        ...
        if issame
            ...
            elseif N == Float32
                ...
                @test typeof(Z2c) == typeof(Z2a) && Z2c.center == c2 &&
                    ispermutation(generators(Z2c), eachcol(G2))
            end
        end
    end
