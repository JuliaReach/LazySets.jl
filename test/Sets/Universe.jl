...
for N in @tN([Float64, Float32, Rational{Int}])
    ...
    @test_broken @inferred isempty(U, true)  # TODO make this type-stable (witness)
    ...
    @test_broken @inferred isuniversal(U, true)  # TODO make this type-stable (witness)
    ...
    @static if VERSION >= v"1.11"
        @test_broken @inferred isdisjoint(U, U, true)  # TODO make this type-stable (witness)
    end
    res, w = isdisjoint(U, U, true)
    @test !res && w isa Vector{N} && w ∈ U
    @test (@inferred isdisjoint(U, E)) && (@inferred isdisjoint(E, U))
    @static if VERSION >= v"1.12"
        @test_broken @inferred isdisjoint(U, Pe)  # TODO make this type-stable (witness)
    end
    ...
    @test_broken !(@inferred U ⊂ U)  # TODO make this type-stable (witness)
    @test !(U ⊂ U)
    @test_broken @inferred ⊂(U, U, true)  # TODO make this type-stable (witness)
    res, w = ⊂(U, U, true)
    @test !res && w isa Vector{N} && isempty(w)
    @test !(U ⊂ B)
    res, w = ⊂(U, B, true)
    @test !res && w isa Vector{N} && w ∈ U && w ∉ B
    @test_broken @inferred B ⊂ U  # TODO make this type-stable (witness)
    @test B ⊂ U
    res, w = ⊂(B, U, true)
    @test res && w isa Vector{N} && w ∉ B && w ∈ U
