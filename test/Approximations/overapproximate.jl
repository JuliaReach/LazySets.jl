...
for N in @tN([Float64, Float32, Rational{Int}])
    ...
    @static if isdefined(@__MODULE__, :TaylorModels) && isdefined(@__MODULE__, :TaylorSeries)
        ...
        local x₁, x₂, x₃ = TaylorSeries.variables!(N, ["x₁", "x₂", "x₃"]; order=5)
        ...
        y = TaylorSeries.variables!("y"; numvars=2, order=1)
        p = zero(y[1])
        @test get_linear_coeffs(p) == N[0, 0]

        # auxiliary function to get nonlinear coefficients of TaylorN
        x = TaylorSeries.variables!("x"; numvars=2, order=10)
        ...
        x = TaylorSeries.variables!("x"; numvars=2, order=1)
        ...
        local x₁, x₂ = TaylorSeries.variables!(N, ["x₁", "x₂"]; order=5)
