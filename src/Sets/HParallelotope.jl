...
``2n`` pairwise parllel constraints. Every parallelotope is a zonotope. As such, parallelotopes
can be represented either in constraint form or in generator form. The `HParallelotope` type
represents parallelotopes in constraint form.

The constraint form is described next.
Let ``D ∈ \\mathbb{R}^{n × n}`` be a matrix and let ``c ∈ \\mathbb{R}^{2n}`` be a vector.
The parallelotope ``P ⊂ \\mathbb{R}^n`` generated by the directions matrix ``D`` and
the offset vector ``c`` is given by the set of points
...
A matrix with the directions of the parallelotope.
"""
function directions(P::HParallelotope) where {N<:Real}
    return P.directions
end
...
A vector with the offsets of the parallelotope.
"""
...
system ``D_i x = c_{n_i}`` for ``i = 1, \\ldots, n``, see [1, Section 3.2.1].
...
matrix ``D`` and offset vector ``c``.
The "extremal vertices" of ``P`` with respect to its base vertex can be
computed as the solution of the ``n``-dimensional linear systems of equations
``D x = v^i`` where for each ``i = 1, \\ldots, n``,
...
```math
    c = q + \\sum_{i=1}^n \\frac{1}{2}(v_i - q) = q(1-n/2) + \\frac{s}{2},
```
...
Let ``P`` be a parallelotope with base vertex ``q`` and list of extremal
vertices with respect to ``q`` given by the set ``\\{v_i\\}`` for
``i = 1, \\ldots, n``. Then, the ``i``-th column of the matrix of generators ``G``
is given by
