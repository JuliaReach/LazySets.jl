...
the basis of the star set, and ``P : α ∈ \\mathbb{R}^m → \\{⊤, ⊥\\}`` where
``P(α) = ⊤`` if and only if ``Aα ≤ b`` is the polyhedral predicate.
...
```jldoctest star_constructor
julia> S = Star(c, V, P)
Star{Float64,Array{Float64,1},Array{Float64,2},BallInf{Float64,Array{Float64,1}}}([3.0, 3.0], [1.0 0.0; 0.0 1.0], BallInf{Float64,Array{Float64,1}}([0.0,
0.0], 1.0))
```
...
BallInf{Float64,Array{Float64,1}}([0.0, 0.0], 1.0)
```
In this case, we know calculating by hand that the generalized star ``X``
defines the rectangular set:

```math
 Θ = \\{(x, y) ∈ \\mathbb{R}^2 : (2 ≤ x ≤ 4) ∧ (2 ≤ y ≤ 4) \\}
```
We can check that assertion:

```julia
julia> Θ = Hyperrectangle(low=[2, 2.], high=[4, 4.])
Hyperrectangle{Float64,Array{Float64,1},Array{Float64,1}}([3.0, 3.0], [1.0, 1.0])

julia> isequivalent(X, Θ) # FIXME
true
```
