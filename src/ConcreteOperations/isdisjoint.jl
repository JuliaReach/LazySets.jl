...
@validate_commutative function isdisjoint(X::LazySet, Y::IA.Interval, witness::Bool=false)
    return isdisjoint(X, Interval(Y), witness)
end

@validate_commutative function isdisjoint(X::LazySet, Y::IA.IntervalBox, witness::Bool=false)
...
for T in [:AbstractZonotope, :AbstractSingleton]
    ...
    # disambiguation
    @eval @validate_commutative function isdisjoint(::CartesianProduct{N,<:Universe,<:Universe},
                                                    Z::$(T)) where {N}
        return false
    end
end
