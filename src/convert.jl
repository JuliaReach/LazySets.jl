...
"""
    convert(::Type{HPolytope}, Z::Zonotope{N}) where {N<:Real}

Convert from zonotope to polytope in H-representation.
...
The zonotope represented as polytope in H-representation.
...
"""
function convert(::Type{HPolytope}, Z::Zonotope{N}) where {N<:Real}
    p = ngens(Z)
    n = dim(Z)
    if p < n
