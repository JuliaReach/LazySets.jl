...
"""
    rectify(Z::AbstractZonotope{N}) where {N}

Concrete rectification of a zonotopic set.
...
The zonotope obtained by rectifying `Z`.
...
"""
function rectify!(Z::AbstractZonotope{N}) where {N}
    c = Z.center
    G = Z.generators
    n, m = size(G)
...
rectify(Z::AbstractZonotope) = rectify!(copy(Z))
